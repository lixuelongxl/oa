...
import net.openmob.server.event.ServerEventListener;
import net.openmob.server.protocal.Protocal;
...
public class ServerEventListenerImpl implements ServerEventListener
{
	...
	public boolean onTransBuffer_C2S_CallBack(Protocal p, IoSession session)
	{
		...
		logger.debug("【DEBUG_回调通知】[typeu="+typeu+"]收到了客户端"+from_user_id
				+"发给服务端的消息：str="+dataContent);
		return true;
	}
